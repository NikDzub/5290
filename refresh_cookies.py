...
async def p1():
    for index, acc in enumerate(cookies_json):
        print(f"{acc}---{index}/{len(cookies_json)}")
        async with async_playwright() as p:
            ...
            await context.contexts[0].add_cookies(
                json.loads(Path(f"etc/cookies/{acc}").read_text())
            )
            await page.goto(
            ...
            try:
                ...
                with open(f"etc/cookies_updated/{acc}", "w") as outfile:
